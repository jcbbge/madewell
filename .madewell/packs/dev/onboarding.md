...
3. **Confirm the lane.** Name exactly what's theirs to work on (their active tasks), what's out of bounds, and the rule: anything that wants to change architecture or convention is a *finding to surface to the Lead*, not a unilateral move.
4. **First safe contribution.** Point them at one well-scoped task with a clear exemplar to match. They make it; it runs the lifecycle and Lands. The loop is now theirs.
