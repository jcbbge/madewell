...
> **Scope:** orchestration — a fan-out pattern per act (`MADEWELL.md` →
> "Orchestration — the recursive coordination layer"). Covers **both loops**: the **outer-loop
> fleet** of pieces and the full inner pass — ideate, plan, implement + verify (Steps 1–6, the
...
## Many pieces at once *(the shop)*

When Commit green-lights more than one Discovery item at once, run them as a **fleet** of concurrent
pieces. Each piece taken to the bench gets its own `bench/<slug>/` and runs its own inner pass independently.
**Default to one piece at a time**; any width beyond one is the Lead's explicit call. The outer
...
2. **Claim by structure, not by a side-file.** A piece on the bench has exactly one pair of
   ...
   **Claims expire — a crashed piece must not hold its files forever:**
   - Every claim carries `expires` (default: now + 30 minutes). A piece still working past its
   ...
   - Taking over an expired claim is explicit, never silent: append
     `{"ts","piece","claims":[paths],"kind":"takeover","from":"<dead-piece>"}`, then proceed.
     Before building on the dead piece's partial work, Verify it — a takeover inherits files,
     never trust.
   - On finish (or on abandoning a target), a piece MUST append
     `{"ts","piece","claims":[paths],"kind":"release"}`. Expiry is the safety net, release is
     the discipline.
3. **Share findings as they happen.** A discovery in one piece that changes another's plan MUST be
   posted to the board immediately (`"kind":"finding"`), not held to the end. pieces MUST read the
   board between phases.
4. **Reconcile at finish.** Each piece finishs on its own. The outer orchestrator MUST merge results in
   claim order. If two pieces touched the same file (the partition should make this impossible),
...
Isolation holds per piece — each carries its own planner/implementer/verifier separation. The board
schema and partition heuristics will tighten with real runs; the rules above are the floor, not the
ceiling.

---

## ideate — parallel understanding *(inner-loop take-in)*

When a piece taken to the bench isn't fully understood, fan out **read-only** understanders before shaping the ideate queue:
...
3. **Shape** the result into the ideate queue — the smallest completable items.
4. **Pause** — surface "here's what this is, and the pieces" to the person before moving to Plan.

Skip the fan-out for a small or already-clear item — ideate it inline. Isolation holds: the understanders are not the implementers.
...
4. **Output** the dependency graph: set `dependsOn` on every ideate item. Items with no deps are the starting frontier; items that share no dependency relationship can run concurrently. **Pause** before Make.
...
## implement + verify — parallel execution
...
4. CROSS-IMPACT CLAUSE — if your verdict implies a fix, enumerate which
   ...
   or the fix invalidates green checks and costs an extra triage piece.
...
1. **Event log wins** over .madewell/stock/ on any conflict.
...
4. **For non-code packages** (`Testing: Applies: no`), the lifepiece collapses
