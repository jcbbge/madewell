...
Source of the folds: Lauren Tan's pstack SOPs, taken as law for a cut, not as a second process. No slash commands. No sticky mode. No plugin. The Cursor desk may still run those tools; this trade does not name them.
...
- **Teach it once.** One plain account: what it is, how it runs, why that shape. Not a file tour.
...
The floor is still four lines. This sitting tightens what those lines *mean* for software:

- **Making** names the change in terms of the caller's use, the types, and the boundary it crosses. Not a function list.
- **Not making** names the shapes you refused, not just the tickets you deferred.
- **Done when** names a real artifact someone can run or inspect. "It compiles" is not done-when.
...
- Smallest change that solves the floor. Delete dead path before adding a new one.
...
- Hand the diff to a mind that did not make it. They try to break it. They sort findings: act / consider / noted / dismissed. Dismissed stays visible.
...
A software piece moved through the four acts with the matching paragraph applied at that act — not as a checklist at the end.
